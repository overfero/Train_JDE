...
class NVDECVideoCapture:
    """Drop-in pengganti cv2.VideoCapture yang pakai NVDEC via PyAV.
    
    vid_stride TIDAK diterapkan di sini — diterapkan di LoadImagesAndVideos
    supaya frame yang di-skip benar-benar tidak di-decode (pakai discard).
    """
    ...
    def __init__(self, path: str):
        ...
        # Buka container sekali, baca metadata dari situ
        # Options untuk NVDEC: gpu=0 pilih GPU pertama
        # Coba hw codec dulu, fallback ke SW
        container_sw = av.open(path)
        vs = container_sw.streams.video[0]
        self.fps = float(vs.average_rate) or 25.0
        self.frame_count = vs.frames or 0
        codec_name = vs.codec_context.name
        self.width = vs.width
        self.height = vs.height
        container_sw.close()

        hw_codec = CODEC_MAP.get(codec_name, None)

        # Coba NVDEC
        self._hw_codec = codec_name  # default fallback
        if hw_codec:
            try:
                ...
                # Paksa decode pakai cuvid codec
                vs2 = self._container.streams.video[0]
                vs2.thread_type = "AUTO"  # multithreaded decode
                self._hw_codec = hw_codec
            except Exception:
                if self._container:
        ...
        if self._container is None:
            ...
            vs2 = self._container.streams.video[0]
            vs2.thread_type = "AUTO"  # multithreaded SW decode
            self._hw_codec = codec_name

        self._video_stream = self._container.streams.video[0]
        self._frame_iter = self._container.decode(self._video_stream)
        self._opened = True
    ...
    def grab(self) -> bool:
        """Ambil satu frame berikutnya. vid_stride diurus di level atas."""
        try:
            self._last_frame = next(self._frame_iter)
            return True
    ...
    def retrieve(self) -> tuple[bool, np.ndarray | None]:
        """Convert frame PyAV terakhir ke numpy BGR."""
        if self._last_frame is None:
            return False, None
        # to_ndarray dengan format bgr24 → konversi di CPU (libuv/FFmpeg)
        # Ini sudah dioptimasi FFmpeg (SIMD), jauh lebih cepat dari manual
        img = self._last_frame.to_ndarray(format="bgr24")
        return True, img

    def get(self, prop_id: int) -> float:
        """Emulasi cv2.VideoCapture.get() untuk properti umum."""
    ...
    @property
    def hw_codec(self) -> str:
        return self._hw_codec


class LoadImagesAndVideos:
    """LoadImagesAndVideos dengan NVDEC untuk video decode."""
    ...
    def __next__(self) -> tuple[list[str], list[np.ndarray], list[str]]:
        ...
        while len(imgs) < self.bs:
            ...
            if self.video_flag[self.count]:
                ...
                # Terapkan vid_stride di sini: grab N frame, keep yang terakhir
                # Frame yang di-skip tetap di-decode (H264 tidak bisa skip B/P frame)
                # tapi setidaknya kita kontrol berapa decode per iterasi
                success = False
                for _ in range(self.vid_stride):
                    success = self.cap.grab()
                    if not success:
                ...
                if success:
                    ...
                    if success:
                        ...
                        info.append(
                            ...
                            f"(frame {self.frame}/{self.frames}) "
                            f"[{self.cap.hw_codec}] {path}: "
                        )
    ...
    def _new_video(self, path: str):
        """Buat NVDECVideoCapture baru untuk path video."""
        self.frame = 0
        self.cap = NVDECVideoCapture(path)  # vid_stride diurus di __next__
        if not self.cap.isOpened():
            raise FileNotFoundError(f"Failed to open video {path}")
        self.fps = self.cap.fps
        self.frames = int(self.cap.frame_count / self.vid_stride) if self.vid_stride > 1 else self.cap.frame_count
        LOGGER.info(f"   📹 {Path(path).name} → codec: {self.cap.hw_codec} | "
                    f"{self.cap.width}x{self.cap.height} @ {self.fps:.1f}fps | "
